...
export interface Payload {
  ...
  totais?: { cidades: number; nominais: number; brancos: number; nulos: number; total: number; secoes: number; secoesTotais: number; desde: string; semTotais: number; secoesEstado: number; secoesApuradasEstado: number };
}
...
export function prepare(p: Payload): OfficeData {
  ...
  return {
    ...
    totais: p.totais ?? { cidades: 0, nominais: 0, brancos: 0, nulos: 0, total: 0, secoes: 0, secoesTotais: 0, desde: '', semTotais: 0, secoesEstado: 0, secoesApuradasEstado: 0 },
...
export function empty(office: Office, status: Status, message: string, loaded = 0, total = 0): OfficeData {
  return { office, area: office === 'president' ? 'BR' : UF, status, message, loaded, total, version: -1, sourceAt: null, nums: [], totals: [], names: new Map(), cities: [], byCdi: new Map(), wins: new Map(), totais: { cidades: 0, nominais: 0, brancos: 0, nulos: 0, total: 0, secoes: 0, secoesTotais: 0, desde: '', semTotais: 0, secoesEstado: 0, secoesApuradasEstado: 0 } };
}
