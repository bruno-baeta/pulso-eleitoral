...
test('arquivo municipal: layout de 2022 (abr → cand) e de 2026 (carg → agr → par → cand)', () => {
  ...
  const lido = parseMunicipal(v2022, { election: '546', cd: '41238', office: 'governor', uf: 'MG' });
  assert.equal(lido.vv, 100);
  assert.deepEqual(lido.cand, [['30', 60], ['55', 40]], 'candidatura com zero voto fica de fora');
  assert.equal(lido.sourceAt, null);
...
test('o arquivo municipal também entrega brancos, nulos e seções', () => {
  /*
   * São os campos que permitem somar as cidades e conferir o total da disputa por outro caminho.
   * Vêm publicados pelo TSE em cada arquivo — nada aqui é derivado.
   */
  const u2026 = {
    ele: '21272', cdabr: '41238',
    v: { vv: '911929', vb: '94492', vn: '93531', tv: '1286492' },
    s: { ts: '4927', st: '3794' },
    carg: [{ cd: '3', agr: [{ par: [{ cand: [{ n: '10', vap: '30' }] }] }] }],
  };
  const lido = parseMunicipal(u2026, { election: '21272', cd: '41238', office: 'governor', uf: 'MG' });
  assert.equal(lido.vv, 911929);
  assert.equal(lido.vb, 94492);
  assert.equal(lido.vn, 93531);
  assert.equal(lido.tv, 1286492);
  assert.equal(lido.st, 3794);
  assert.equal(lido.ts, 4927);

  // Arquivo sem os totais não quebra: os campos vêm zerados e a soma sabe excluir a cidade.
  const magro = parseMunicipal({ ele: '21272', cdabr: '41238', v: { vv: '10' }, carg: [{ cd: '3', agr: [] }] },
    { election: '21272', cd: '41238', office: 'governor', uf: 'MG' });
  assert.equal(magro.ts, 0);
});
